...
"""Class to work on quality estimators applied to iris"""

class QualitySystemIris:
    ...
    def normalisation(self, info):
        ...
        for id,eye,v in zip(list(Id_list),list(Eyes_list),list(scores)):

            key = str(id)  + eye

            # full_name = name.split('.jpg')[0]
    ...
    def building_enrol_random(self,dataset):
    ...
    def bins_visited(self,score_q_s, labels_s, model):
        ...
        for score_probe, label in zip(score_q_s,labels_s):
            ...
            if len(list_bin_visited) > 0:

                count_bin_high = len(list_bin_visited)

                list_higher_bin.append(count_bin_high)

                list_higher_cost.append(0)

        return list_higher_bin, list_higher_cost
