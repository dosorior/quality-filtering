...
from enrolment.FingerprintQualitySystem import QualitySystemFingerPrint
import os
import json
...
"""Quality filtering applied to the Fingerprint-based source """
...
save_path_csv = os.path.join(args.output, "Finger_DP_Statistics_{}.csv".format(args.name))

path_scores_total = os.path.join(args.output, "finger_scores_q_total_{}.csv".format(args.name))

path_scores_used = os.path.join(args.output, "finger_scores_q_used_{}.csv".format(args.name))
...
with open(save_path_csv, 'w', newline='') as f:
    ...
    for round in range(0,10):
        ...
        enrol_model = QualitySystemFingerPrint()

        penetration_rate = 0

        #loading info_json
        path_file_json = os.path.join(args.model,'MCYT330_dp.csv')
        ...
        selected_subjects = gettingdata(dataset)

        selected_subjects = list(selected_subjects)

        score_q_s, labels_s, count_e, list_scores_q_used = enrol_model.building_enrol_random(selected_subjects,dataset)
